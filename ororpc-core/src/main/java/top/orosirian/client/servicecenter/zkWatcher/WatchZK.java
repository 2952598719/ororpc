                ...
                switch(type.name()) {
                    ...
                    case "NODE_CREATED":    // 得到创建节点的通知，直接放入缓存，此时oldData为空
                        ...
                        if(pathToCreate.length <= 2) {
                            ...
                            String interfaceName = pathToCreate[1];
                            String serverAddr = pathToCreate[2];
                            cache.addService(interfaceName, serverAddr);
                            log.info("节点创建：服务名称 {} 地址 {}", interfaceName, serverAddr);
                        }
                        break;
                    case "NODE_CHANGED":    // 得到节点更新的通知，删除旧的，放入新的
                        ...
                        cache.replaceServiceAddress(oldPaths[1], oldPaths[2], newPaths[2]);
                    ...
                    case "NODE_DELETE":     // 得到节点删除的通知，此时data为空
                        ...
                        if(pathToDelete.length <= 2) {
                            ...
                            String interfaceName = pathToDelete[1];
                            String serverAddr = pathToDelete[2];
                            cache.deleteServiceAddress(interfaceName, serverAddr);
                            log.info("节点删除：服务名称 {} 地址 {}", interfaceName, serverAddr);
                        }
                        break;
                    default:
                        break;
